...
export default function OrbiLogo({ size = 32, showText = true, className = '', color = 'dark' }: OrbiLogoProps) {
  ...
  if (!showText) {
    return (
      <svg viewBox="0 0 48 48" width={size} height={size} className={className}>
        ...
        <ellipse cx="24" cy="24" rx="22" ry="9" fill="none" stroke={c} strokeWidth="0.5" opacity="0.25" />
        <circle cx="46" cy="24" r="2.5" fill={accent} />
      </svg>
    )
  }
  ...
  return (
    <svg viewBox="0 0 140 40" height={size} className={className}>
      ...
      <ellipse cx="16" cy="20" rx="15" ry="7" fill="none" stroke={c} strokeWidth="0.5" opacity="0.2" />
      <circle cx="31" cy="20" r="2" fill={accent} />
      <text x="40" y="26" fontFamily="'Source Serif 4', Georgia, serif" fontSize="20" fontWeight="400" fill={c} letterSpacing="-0.5">
        orbbi
      </text>
    </svg>
  )
